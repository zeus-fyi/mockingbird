...
import {SchemasRow} from "./SchemasRow";

export function SchemasTable(props: any) {
    const {selected, schemas, handleClick, handleSelectAllClick} = props;
    ...
    const emptyRows =
        page > 0 ? Math.max(0, (1 + page) * rowsPerPage - schemas.length) : 0;
    if (emptyRows) {
        return (<div></div>)
    }
    if (loading) {
            ...
            <Table sx={{ minWidth: 1000 }} aria-label="actions pagination table">
                <TableHead>
                    <TableRow style={{ backgroundColor: '#333'}} >
                        ...
                        <TableCell style={{ fontWeight: 'normal', color: 'white'}} >Schema ID</TableCell>
                ...
                <TableBody>
                    {rowsPerPage > 0 && schemas && schemas.map((row: any, index: number) => (
                        <SchemasRow
                            key={index}
                            row={row}
                            index={index}
